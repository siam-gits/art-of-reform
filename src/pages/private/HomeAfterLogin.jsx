...
const HomeAfterLogin = () => {
  ...
  useEffect(() => {
    // 1. Force the browser memory to 'true'
    localStorage.setItem("isLoggedIn", "true");

    // 2. IMPORTANT: If the navbar still shows "Login",
    // it's because the Layout hasn't re-rendered.
    // We check if we need to refresh the state.
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {[1, 2, 3, 4, 5, 6].map((item, idx) => {
            ...
            const artData = {
              ...
              dimension: '18" x 24"',
              description:
                "A profound exploration of perspective and confinement...",
            };

            return (
              <div
                key={idx}
                className="relative group bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden"
              >
                {/* Image Container */}
                <div
                  ...
                  onClick={() => !isSold && openArtDetails(artData)}
                >
                  <img
                    src={heroArt1}
                    alt="Artwork"
                  ...
                  {isSold && (
                    <div className="absolute inset-0 flex items-center justify-center p-6 z-10">
                      <div className="bg-[#FF0000] text-white text-4xl md:text-5xl font-black px-10 py-5 rounded-2xl transform -rotate-[15deg] shadow-[0_20px_50px_rgba(0,0,0,0.3)] border-4 border-white/10 tracking-tight">
                        SOLD
                      </div>
                    </div>
                  )}
                ...
                <div className="p-6 space-y-4 text-gray-900">
                  <div>
                    <h3 className="text-xl font-bold">Behind the Wall</h3>
                    <p className="text-sm opacity-70">by Patrick Moore</p>
                  </div>

                  <div className="flex justify-between items-center border-t border-gray-100 pt-4">
                    <span className="text-sm opacity-60">Buy It Now</span>
                    <span
                      className={`text-xl font-bold ${
                        isSold ? "text-[#4ade80]" : "text-[#27AE60]"
                      }`}
                    >
                      $775
                    </span>
                  ...
                  {!isSold && (
                    <div className="flex gap-3 pt-2">
                      <button
                        ...
                        className="flex-1 py-3 bg-[#2F6DF2] text-white rounded-lg text-sm font-bold hover:brightness-110 transition"
                      >
                        Make a Bid
                      </button>

                      <button className="flex-1 py-3 bg-[#00B14F] text-white rounded-lg text-sm font-bold hover:brightness-110 transition">
                        Buy Now
                      </button>
                    </div>
                  )}
